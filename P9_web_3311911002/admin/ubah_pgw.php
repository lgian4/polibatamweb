...
$result = mysqli_query($koneksi, "SELECT * FROM pegawai WHERE nik='$nik'"); while($user_data = mysqli_fetch_array($result)) 
{ 
    $nama = $user_data['nama']; 
...
target="#navbarSupportedContent" aria-controls="navbarSupportedContent" aria-expanded="false" arialabel="Toggle navigation"> 
 ...
 <form action="update_pgw.php" method="post"> 
  ...
  <div class="form-row"> 
     <div class="form-group col-md-4">       <label >NIK</label> 
       <input type="text" name="nik" class="form-control" id="nik" value=<?php echo $nik;?>> 
    </div> 
   </div> 
    <div class="form-row"> 
    <div class="form-group col-md-8">        <label >NAMA</label> 
      <input type="text" name="nama" class="form-control" id="nama" value=<?php echo $nama;?>>      </div> 
  </div> 
 
 <div class="form-row"> 
     <div class="form-group col-md-6">       <label >BAGIAN</label> 
